...
import { readJsonIfExists } from '../payload/scripts/lib/e2e-root.mjs';
...
function normalizeStored(value) {
  try {
    ...
    throw new PathError(err.message);
  }
}

export function normalizeE2eRoot(value) {
  const normalized = String(value).trim().replace(/\\/g, '/').replace(/\/+/g, '/').replace(/^(?:\.\/)+/, '').replace(/\/+$/, '');
  if (!normalized) throw new UsageError('E2E ルートが空です');
  if (normalized.startsWith('/') || /^[A-Za-z]:/.test(normalized)) {
    throw new UsageError(`E2E ルートは target 相対で指定してください: ${value}`);
  }
  if (normalized.split('/').includes('..')) throw new UsageError(`E2E ルートに '..' は使えません: ${value}`);
  return normalized;
}
...
// Keep the exported legacy option compatible without rehashing immutable baselines per file.
let legacyTransformIndex;

export function transformBytes(rel, buf, e2eRoot, { legacy = false } = {}) {
  if (e2eRoot === E2E_ROOT_DEFAULT || buf.includes(0)) return buf;
  if (legacy && !(legacyTransformIndex ??= loadLegacyIndex()).byPath.get(rel)?.some(entry => entry.transformE2eRoot)) return buf;
...
function resolvePlacement(target, flagRoot) {
  ...
  else if (current.data?.e2eRoot) { root = normalizeStored(current.data.e2eRoot); how = `${STAMP_FILE} の記録`; }
  else if (!legacy.broken && legacy.data?.e2eRoot) { root = normalizeStored(legacy.data.e2eRoot); how = `${LEGACY_STAMPS.e2e} の記録`; }
  else if (detected) { root = detected; how = detectedHow; }
  const previous = !current.broken && current.data?.e2eRoot ? normalizeStored(current.data.e2eRoot) : (!legacy.broken && legacy.data?.e2eRoot ? normalizeStored(legacy.data.e2eRoot) : null);
  return { root, how, detected, configs, notes, previous, current };
}

function knownLegacyMatch(existing, rel, e2eRoot, legacyIndex, stamps) {
  const entries = legacyIndex.byPath.get(rel) ?? [];
  for (const entry of entries) {
    const stamp = stamps[entry.source.id];
    if (!stamp || stamp.broken || stamp.version !== entry.source.packageVersion) continue;
    const expected = entry.transformE2eRoot ? transformBytes(rel, entry.bytes, e2eRoot) : entry.bytes;
...
export async function main(argv = process.argv.slice(2), io = {}) {
  ...
  for (const rel of payloadFiles) {
    ...
    else if (knownLegacyMatch(existing, rel, placement.root, legacyIndex, stamps)) action = 'migrate';
