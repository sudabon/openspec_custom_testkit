...
const LEGACY_E2E_TRANSFORM = new Set([
...
export function transformBytes(rel, buf, e2eRoot, { legacy = false } = {}) {
  ...
  if (legacy && !LEGACY_E2E_TRANSFORM.has(rel)) return buf;
...
function knownLegacyMatch(existing, rel, e2eRoot, legacyIndex, stamps) {
  ...
  for (const entry of entries) {
    ...
    const expected = transformBytes(rel, entry.bytes, e2eRoot, { legacy: true });
